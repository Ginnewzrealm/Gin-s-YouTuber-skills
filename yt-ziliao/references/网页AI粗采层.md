...
## 六、文书员模式（步骤 5'：网页 AI 精读 + 结构化，v2.10.0）

> 粗采层（步骤 4.7）负责"找哪几页"（侦察兵）；文书员负责"读这几页并变成 materials 条目"（文书员）。两者串行，互为补充。

### 定位

输入 = 粗采层/子 Agent 给出的**已验证 URL 清单**（10-20 个）；输出 = `raw/webai_structured.json`（网页 AI 的结构化原始记录）→ 子 Agent 抽验 + 双源核验 → `materials.json`。

**省 Token 的本质**：原来"子 Agent 每页 WebFetch 读全文 + 手写条目"→ 现在"网页 AI 免费读 + 吐 JSON，子 Agent 只抽验 20%"。

### 提示词模板（强约束，实测有效）

```
请打开并阅读：<URL>
严格按原文提取，不要加工、改写、总结、推测。逐字段输出JSON：
{"title": "原文标题原文照抄", "publish_date": "YYYY-MM-DD或null", "platform": "网站名",
 "key_facts": ["原文明确写出的事实，逐字或近逐字照抄，含数字/日期/人名优先，≤8条"],
 "direct_quotes": ["人物直接引语，带引号逐字照抄"]}
规则：1) 只输出JSON本体；2) 原文没有的字段填null或空数组；3) 绝对禁止填原文没有的内容；
4) 无法访问该网页或正文为空，输出 {"error": "unreadable"}，不要硬编内容。
```

**逐页轮换 provider**（遵守 §二 频率控制）；每页调用记录 receipt 到 `webai_structured.json`。

### 子 Agent 精加工（只做三件事）

1. **抽验 20%**：随机挑条目，curl 抽正文核对关键事实未被改写/编造；变形率 >10% → 该批次作废
2. **双源核验**：数字/引语走 §四 双源制（对 AI 和人工一视同仁）
3. **立场/权威标注**：你方读 key_facts 判断，不信 AI 自评

### 三重降级开关

| 层级 | 触发 | 行为 |
|---|---|---|
| 配置级 | `webai_structured: false` | 永久回退纯 WebFetch 模式 |
| 运行时级 | CDP 不通 / smoke 全 UNREACHABLE | 本任务自动跳过，走 WebFetch |
| 质量级 | 抽验变形率 >10% 或单页返回 `{"error":"unreadable"}` | **该页踢到 WebFetch 降级队列**（精准降级，不整批作废）；整批变形率超标才整批回退 |

### 已知输出噪声（解析时剔除）

- 思考过程/trace 文本（Kimi 常见）
- `-1` `-10` 类脚注残留（DeepSeek 常见）
- 末尾无意义后缀（如 `-1-2-3`）
- 图片下载产物（`ai-image-*.png`，须删除，已 gitignore）

### 实测记录（2026-09-10，单样本）

DeepSeek + 强约束 prompt 提取搜狐文章：8 项关键字段全忠实原文，无编造；publish_date 从页面元数据取到（比我方手工登记的 2017-01-01 更准）；`direct_quotes` 空属忠实（该页确实无引语）。**意外发现我方 v1.1 手工条目存在污染**（该页无 5/15/15/15 明细却被记入）——说明双源核验须对 AI 和人工一视同仁。**局限：单样本单 provider，试点期须 3-5 个不同来源类型验证。**
