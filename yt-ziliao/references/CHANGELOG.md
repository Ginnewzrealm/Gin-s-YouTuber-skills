...
### v2.7.0 — 商业拆局挖掘手册 v2（三层颗粒 × 五维信源 × 检索语法）

> 背景：频道定位升级为"买方视角商业极境拆局纪录片"（见 ~/Downloads/YouTuber技能优化调整方案.md P0①）。
> 通用六章采集对"拆局"类选题没有"怎么挖"的地图——考古信源在哪、检索语法怎么写、空方弹药怎么收，全靠临场发挥。
> 本版把 2026-09 归档的三份方法论材料（极境考古手册/微观颗粒打法/Agent 执行手册）整合为挖掘手册-v2，挂在步骤 5。
> **性质：纯文本层增量——六章骨架/审计链/manifest 脚本/状态机全部不动。**

| 变更 | 说明 | 文件 |
|------|------|------|
| 新增手册 | `挖掘手册-v2.md`：三层颗粒（人/产品/公司关卡）× 五维信源（经营者考古/产品PMF/对手审计/投资人复盘/空方证据包）× 检索语法；SEC 硬伤修正为港股/A股/SEC 三套招股书；维度五 b（Kill_Thesis 推导）明令归 yt-fenxi | references/挖掘手册-v2.md |
| 约束条 | 五条（v2.7.0）：禁主观定性词/每条标信源类型/锚定可追溯证据/五维封闭+推导不越界/归因纪律（早年经历只记呼应不做心理诊断） | 挖掘手册-v2.md §〇 |
| 流程挂载 | 步骤 5：拆局类选题四路子 Agent 按手册三层×五维组织检索内容（运输层不变）；输出契约加 §5.4 锚点清单行（fenxi N6 消费） | SKILL.md |
| 模板 §5.4 | 候选锚点清单（物件/数字/对话三锚点各≥1 或标暂缺），只盘点不评估 | references/report-template.md |
| 写作原则 | 加第 5 条：拆局类选题禁主观定性词；自检清单加两项（锚点清单/定性词） | references/report-template.md |
| 明确不改 | 六章骨架保留（手册是透镜不是替换）；审计链/manifest 脚本不动；anchors 不进 manifest（可选字段暂不启用） | — |
