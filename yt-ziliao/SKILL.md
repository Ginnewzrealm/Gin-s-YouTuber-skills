...
> 被 youtube-skills（core）调用时：不输出宏观进度（宏观六阶段归 core），按本技能步骤流执行；路由与工作区路径以 core 共享配置为准。
...
- **立场**：支持方/质疑方/中立
- **数字/引语双源制（v2.9.1 制度化）**：数字、日期、直接引语必须两个独立来源印证；单源标"单一来源"降一档，无源进 rejected——硬规则，非口头约定（网页 AI 粗采层线索一律先按"待核验"对待）
