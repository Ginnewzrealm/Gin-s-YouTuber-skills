...
本技能明确不执行以下操作：
- 不代替 yt-ziliao 做资料采集与报告生成
- 不直接操作任何搜索接口或飞书文档接口
- 不修改资料包内容（发现资料疑点只能记入分析卡盲区清单，回采仍走 yt-ziliao）

## 二、依赖检测

检测方式：进入 N2 时检测 yt-ziliao 是否已安装可用。

- 已安装 → 自动调补采（Q4 拍板：不问用户）
- 未安装 → 提醒用户安装；本次分析终止，输出缺失清单与"请先运行 yt-ziliao 补采"的指引，主流程不硬跑
...
| 异常情况 | 处理方式 |
|---|---|
| yt-ziliao 未安装 | 提醒安装 + 终止本次分析，输出缺失清单 |
